 ...
 */
public class PreSplitFoldRunTestNaiveBayesHighVsLow {
    ...
    private Supplier<Model> nbFactory(ArrayList<String> sortedLabels) {
        // HvL: k-1 cumulative-threshold models
        int k = sortedLabels.size();
        return () -> {
            ArrayList<Model> models = new ArrayList<>();
            for (int i = 0; i < k - 1; i++) models.add(new NaiveBayesModel());
            return new HighVersusLow(models, sortedLabels);
        };
    }
    ...
    @Test
    public void testNaiveBayesHighVsLow() throws DiscreteFeaturesNotAllowed {
        Job[] jobs = {
        ...
        for (Job job : jobs) {
            ...
            for (String cond : conditions) {
                ...
                try {
                    res = runner.executeDetailed(job.prefix, cond, nbFactory(job.labels), new Parameter(1));
                } catch (Exception e) {
                ...
                if (dp != null && dp.getConfusionMatrix() != null) {
                    ...
                    System.out.println("=== " + name + "HvL - NaiveBayes");
        ...
        excel.writeToExcel("Results_NB_HvL_LeakageFree_PerFold.xlsx");
        System.out.println(">>> Written: Results_NB_HvL_LeakageFree_PerFold.xlsx");
    }
}
