 ...
 */
public class PreSplitFoldRunTestLinearPerceptronOneVsOne {
    ...
    private Supplier<Model> lpFactory(ArrayList<String> sortedLabels) {
        // OvO: one model per class pair -> k(k-1)/2 models
        int k = sortedLabels.size();
        int nModels = (k * (k - 1)) / 2;
        return () -> {
            ArrayList<Model> models = new ArrayList<>();
            for (int i = 0; i < nModels; i++) models.add(new LinearPerceptronModel());
            return new OneVersusOne(models);
        };
    }
    ...
    @Test
    public void testLinearPerceptronOneVsOne() throws DiscreteFeaturesNotAllowed {
        Job[] jobs = {
        ...
        for (Job job : jobs) {
            ...
            for (String cond : conditions) {
                ...
                if (dp != null && dp.getConfusionMatrix() != null) {
                    ...
                    System.out.println("=== " + name + "OvO - LinearPerceptron");
        ...
        excel.writeToExcel("Results_LP_OvO_LeakageFree_PerFold.xlsx");
        System.out.println(">>> Written: Results_LP_OvO_LeakageFree_PerFold.xlsx");
    }
}
