 ...
 */
public class PreSplitFoldRunTestNaiveBayesOneVsRest {
    ...
    private Supplier<Model> nbFactory(ArrayList<String> sortedLabels) {
        // OvR: one model per class -> k models
        int k = sortedLabels.size();
        return () -> {
            ArrayList<Model> models = new ArrayList<>();
            for (int i = 0; i < k; i++) models.add(new NaiveBayesModel());
            return new OneVersusRest(models);
        };
    }
    ...
    @Test
    public void testNaiveBayesOneVsRest() throws DiscreteFeaturesNotAllowed {
        Job[] jobs = {
        ...
        for (Job job : jobs) {
            ...
            for (String cond : conditions) {
                ...
                if (dp != null && dp.getConfusionMatrix() != null) {
                    ...
                    System.out.println("=== " + name + "OvR - NaiveBayes");
        ...
        excel.writeToExcel("Results_NB_OvR_LeakageFree_PerFold.xlsx");
        System.out.println(">>> Written: Results_NB_OvR_LeakageFree_PerFold.xlsx");
    }
}
